...
Task Build -depends Clean, CreateSharedAssemblyInfo, Rebuild #, Test      no tests yet
...
Task Rebuild {
    $solution_file = "$source_dir\$product.sln"
...
Task Test {
    ...
    $test_assemblies = "$bin_dir\$product.Specs.dll"
...
function Create-Nuspec ($name) {
    ...
    $shared_nuspec = "$nuspec_dir\$product.Shared.nuspec"
...
[assembly: AssemblyCopyright("Copyright 2012-$($now.Year) Pete Klein")]
